    ...
    public class Pager : IHtmlString
    {
        ...
        public PaginationModel BuildPaginationModel(Func<int, string> generateUrl)
        {
            ...
            if (start > 1)
            {
                ...
                if (start > 2)
                {
                    model.PaginationLinks.Add(new PaginationLink { Active = true, DisplayText = "<li><a href='#'>...</a></li>" });
                }
            }
            ...
            else
            {
                ...
                foreach (var paginationLink in model.PaginationLinks)
                {
                    if (paginationLink.Active)
                    {
                        ...
                        {
                            sb.AppendFormat("<li><span style=\"background-color: #428bca;color:#fff\">{0}</span></li>", paginationLink.DisplayText);
                        }
                        ...
                        else
                        {
                            ...
                            linkBuilder.AppendFormat(" href=\"{0}\">{1}</a></li>", paginationLink.Url, paginationLink.DisplayText);
                            sb.Append(linkBuilder.ToString());
                        }
                    }
                    else
                    {
                        sb.AppendFormat("<li><span class=\"disabled\">{0}</span></li>", paginationLink.DisplayText);
                    }
                }
                sb.Append("</ul>");
                return sb.ToString();
